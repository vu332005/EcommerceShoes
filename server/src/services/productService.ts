...
export const getProducts = async (params: ProductFilterParams) => {
  ...
  const query = productRepo
    ...
    .leftJoinAndSelect("variant.tags", "tag", "tag.type = :brandType", {
      brandType: "brand",
    })
  ...
  if (min_price)
    query.andWhere("product.basePrice >= :min", { min: min_price });
  if (max_price)
    query.andWhere("product.basePrice <= :max", { max: max_price });
  if (category_id)
    query.andWhere("product.categoryId = :catId", { catId: category_id });
  if (tag_ids) {
    //
  ...
  /*
  getManyAndCount(): Đây là một hàm cực kỳ tiện lợi của TypeORM. Nó thực hiện 2 câu lệnh SQL ngầm cùng một lúc:
    - Lấy ra mảng dữ liệu sản phẩm (đã bị cắt bởi limit và skip).
    - Đếm tổng số lượng sản phẩm thỏa mãn điều kiện lọc (không bị ảnh hưởng bởi limit/skip) để tính ra tổng số trang (total).
  */
  const [rawProducts, total] = await query.getManyAndCount();

  // Định dạng lại dlieu trả về -> rawProduct có cấu trúc lồng -> để tiện cho fe dùng -> ta sẽ làm phẳng (flat) nó
...
export const getProductDetail = async (id: number) => {
  ...
  // Lấy dữ liệu thô từ DB (Bao gồm cả quan hệ lồng nhau)
  ...
  const allImages = product.variants.flatMap((variant) => {
    ...
    return variant.images.map((img) => ({
      ...
      color: colorTag
        ? { id: colorTag.id, name: colorTag.name, value: colorTag.name }
        : null,
    }));
  });
...
export const createProduct = async (data: any) => {
  const { name, handle, basePrice, description, thumbnailUrl, categoryId } =
    data;
...
export const deleteProduct = async (id: number) => {
  // Lấy danh sách variants trước khi xóa để biết đường xóa cache
  ...
  // Xóa cache Redis sau khi xóa DB thành công
...
export const createVariant = async (productId: number, data: any) => {
  ...
  //color
  if (color) {
    // nếu ng dùng có nhập màu thì ms chạy
  ...
  if (brand) {
    ...
    if (!t) {
      t = await tagRepo.save(tagRepo.create({ name: brand, type: "brand" }));
    }
    tags.push(t);
  }
  ...
  if (images && images.length > 0) {
    // check xem có ảnh nào đc gửi lên k
...
export const updateVariant = async (variantId: number, data: any) => {
  ...
  */
  ...
  if (color || size || brand) {
    ...
    */
    const colorName =
      color || variant.tags.find((t) => t.type === "color")?.name;
    // find or create
    if (colorName) {
      let t = await tagRepo.findOneBy({ name: colorName, type: "color" });
      if (!t)
        t = await tagRepo.save(
          tagRepo.create({ name: colorName, type: "color" }),
        );
      newTags.push(t);
    }
    ...
    if (sizeName) {
      ...
      if (!t)
        t = await tagRepo.save(
          tagRepo.create({ name: sizeName, type: "size" }),
        );
      newTags.push(t);
    }

    const brandName =
      brand || variant.tags.find((t) => t.type === "brand")?.name;
    if (brandName) {
      let t = await tagRepo.findOneBy({ name: brandName, type: "brand" });
      if (!t)
        t = await tagRepo.save(
          tagRepo.create({ name: brandName, type: "brand" }),
        );
      newTags.push(t);
    }
