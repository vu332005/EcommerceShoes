...
export const addToCart = catchAsync(async (req: AuthRequest, res: Response) => {
  ...
  res
    .status(200)
    .json({ status: "success", message: "Đã thêm vào giỏ ", data: result });
});
...
export const removeCartItem = catchAsync(
  async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const result = await cartService.removeCartItemService(
      req.user.id,
      Number(id),
    );
    res
      .status(200)
      .json({ status: "success", message: "Đã xóa sản phẩm123", data: result });
  },
);
